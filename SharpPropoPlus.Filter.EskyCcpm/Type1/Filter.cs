...
{
    [ExportPropoPlusFilter("{C2B2125A-7B26-4C68-824E-D508E5DCA0FD}", "CCPM: E-sky (6ch) Type 1", "E-Sky CCPM 6 channel signal into 4 orthogonal channels")]
    public class Filter : FilterProcessor
    {
        ...
        */
        protected override IJoystickData Process(IJoystickData channels, int max, int min)
        {
            ...
            if (inData[2] > max / 2)
                outData[2] = inData[1] + (int) (0.15 * inData[2]);
            else
                outData[2] = inData[1];

            // Ailerons: Rear servo minus pitch effect (roll correction when power above mid-point)
            if (inData[2] > max / 2)
                outData[3] = (int) (max / 2 + inData[0] - 0.5 * inData[1] - 0.15 * inData[2]);
            else
