...
namespace SharpPropoPlus.Decoder.Walkera
{
  //[Export(typeof(IPropoPlusDecoder))]
  //[ExportMetadata("Type", TransmitterType.Ppm)]
  [ExportPropoPlusDecoder("Walkera", "Walkera WK-2401 pulse processor", TransmitterType.Ppm)]
  public class Program : PulseProcessor, IPropoPlusDecoder
  {
    ...
    private void ProcessPulseWk2401Ppm(int width, bool input)
    {
      ...
      if (Math.Abs(_prevWidth[_datacount] - width) < Constants.PPM_JITTER)
      ...
      var newdata = (int) ((width - Constants.PPMW_MIN)/(Constants.PPMW_MAX - Constants.PPMW_MIN)*1024);
