...
{
  public class JoystickConfigViewModel : BaseViewModel
  {
      ...
      using (var enumerator = new DeviceEnumerator())
      {
        Devices = new ReadOnlyObservableCollection<DeviceInformation>(
          new ObservableCollection<DeviceInformation>(
            enumerator.GetDevices(new []{VjdStat.VJD_STAT_BUSY, VjdStat.VJD_STAT_FREE, VjdStat.VJD_STAT_OWN, }).OrderBy(ob => ob.Id)));
      }
    ...
    public DeviceInformation SelectedDeviceItem
    {
      ...
      set
      {
        ...
        GlobalEventAggregator.Instance.SendMessage(new JoystickChangedEventArgs(_selectedDeviceItem.Id, _selectedDeviceItem.Name, _selectedDeviceItem.Guid));
