...
namespace SharpPropoPlus.Events.Events
{
    using System;
        ...
        where TListener : class
      {
        ...
        marshaller(() =>
        {
          foreach (var o in _listeners)
          {
            ...
            //listener.TryHandle<TListener>(message, out wasThisOneCalled);

            bool wasThisOneCalled = false;

            if (o.Handles<TListener>() || o.HandlesMessage(message))
            {
              o.TryHandle<TListener>(message, out wasThisOneCalled);
            }
            else if (o.ListenerInstance is MulticastDelegate)
            {
              //var method = o.ListenerInstance.GetType().GetMethod("Invoke");
              //method.Invoke(o.ListenerInstance, new []{ message });

              var @delegate = (MulticastDelegate) o.ListenerInstance;

              if (@delegate.Method.GetParameters()[0].ParameterType == message.GetType())
                ((MulticastDelegate) o.ListenerInstance).DynamicInvoke(message);


              wasThisOneCalled = true;
            }
    ...
    public IEventSubscriptionManager AddListener<T>(Action<T> action)
    {
      return AddListener(action, true);
    }
    ...
    public IEventSubscriptionManager AddListener(object listener, bool? holdStrongReference)
      ...
      public IEventSubscriptionManager RemoveListener<T>(Action<T> action)
      {
        _listeners.RemoveListener(action);
        return this;
      }
    ...
    private class ListenerWrapperCollection : IEnumerable<ListenerWrapper>
        {
            ...
            private bool TryGetListenerWrapperByListener(object listener, out ListenerWrapper listenerWrapper)
            {
              lock (_sync)
              {
                if (listener is MulticastDelegate)
                {
                  listenerWrapper =
                    _listeners.Where(w => w.ListenerInstance is MulticastDelegate)
                      .SingleOrDefault(x => Equals((MulticastDelegate) x.ListenerInstance, (MulticastDelegate) listener));
                }
                else
                {
